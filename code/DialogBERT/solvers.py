...
class DialogBERTSolver(object):
    ...
    def evaluate(self, args):
        ...
        with open(os.path.join(eval_output_dir, f"test_results.txt"), 'w') as f_eval:
            f_eval.write(generated_text+'\n')
        return result  
        
        # chat_set = HBertMseEuopDataset(os.path.join("./data/", 'chat_en.h5'), self.model.tokenizer)
        # answer, generated_text = Learner().run_chat(args, self.model, chat_set)
        # eval_output_dir = f"./data/"
        # if args.local_rank in [-1, 0]: os.makedirs(eval_output_dir, exist_ok=True)
        # with open(os.path.join(eval_output_dir, f"chat_results_en.txt"), 'w') as f_eval:
        #     f_eval.write(generated_text+'\n')
        # return answer    
