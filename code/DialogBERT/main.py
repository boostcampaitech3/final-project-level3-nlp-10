...
from prepare_data import binarize
...
def main():
    ...
    parser.add_argument("--dataset", default='ko-dialog', type=str, help="dataset name")
    ## Other parameters
    parser.add_argument("--model", default="DialogBERT", type=str, help="The model architecture to be fine-tuned.") 
    parser.add_argument("--model_size", default="small", type=str, help="tiny, small, base, large")
    parser.add_argument("--language", default="korean", type=str, help= "language, english or chinese or korean")
    ...
    # Evaluation
        if args.local_rank in [-1, 0]:
            ...
            #     string = input('User > ')
            #     string = string.strip()
            #     if len(string) == 0: # 입력이 없으면 중단
            #         break
            #     # string = mt(string, src="ko", tgt="en")
            #     dialogs[0]['utts'].append(('A', string, np.zeros((1,1))))
            #     dialogs[0]['utts'].append(('B', '', np.zeros((1,1))))
            #     # print(dialogs)
            #     # 사용자의 입력 chat.h5 파일로 만듦
            #     chat_out_path = os.path.join("./data/", "chat_en.h5")
            #     chat_data_binary=binarize(dialogs, tokenizer, chat_out_path)
            #     # chat.h5로 답변 생성
            #     answer = solver.evaluate(args)
            #     # answer = mt(answer, src="en", tgt="ko")
            #     print(f'Chatbot > {answer}')
            #     # 답변을 dialog에 추가
            #     dialogs[0]['utts'].pop()
            #     dialogs[0]['utts'].append(('B', answer, np.zeros((1,1))))
            #     print(dialogs)
