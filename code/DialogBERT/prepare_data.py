...
def get_emotion_dialog(data_path):
    # json_file_path = "./감성대화말뭉치(최종데이터)_Training.json"

    data = json.load(open(data_path, 'r'))
    dials = list(map(lambda x: list(x['talk']['content'].values()), data))
    dialogs = []
    for dial in dials:
        utts = []
...
def load_data(data_path, data_name):
    ...
    elif args.data_set=='ko-dialog':
        data['train'] = get_emotion_dialog(data_path+'감성대화말뭉치(최종데이터)_Training.json')
        data['valid'] = get_emotion_dialog(data_path+'감성대화말뭉치(최종데이터)_Validation_mini.json')
        data['test'] = get_emotion_dialog(data_path+'감성대화말뭉치(최종데이터)_Validation.json')
    return data


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('-d', "--data_set", default='ko-dialog', help='multiwoz, dailydial, ko-dialog')
    parser.add_argument('-m', "--model_name", required=True, default= 'bert-base-multilingual-uncased', help='bert-base-uncased')
