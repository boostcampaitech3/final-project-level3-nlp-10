...
def app() :
    ...
    file_path = '/opt/ml/streamlit/eda/data/answer.csv'
    ...
    if option == '문장 길이 분포' :
        ...
        if cat_option == '소주제' :
            ...
            with st.container() :
                st.subheader('Category 분포')
                start_rows,end_rows = st.slider(
                    'Select a range of values',0, 174, (0, 30))
                st.write('start row :',start_rows, 'end row :',end_rows )
                #st.bar_chart(count_cat[start_rows:end_rows])
                if start_rows == end_rows :
                    if start_rows == 0:
        ...
        if cat_option == '대주제' :
            ...
            with st.container() :
                st.subheader('Category 분포')
                start_rows,end_rows = st.slider(
                    'Select a range of values',0, 11, (0, 11))
                st.write('start row :',start_rows, 'end row :',end_rows )
                #st.bar_chart(count_cat[start_rows:end_rows])
                if start_rows == end_rows :
                    if start_rows == 0:
